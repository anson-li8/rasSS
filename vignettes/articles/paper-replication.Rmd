...
*The main simulation study, replicating the design of Jiang & Zhang (2025) on a simulated 1,000-SNP chromosome. Pilot and validation on the 300-SNP toy: [Pilot Study](01_simulation.html), [Validation](02_simulation.html).*
...
```{r full_grid, cache=TRUE, results='hide', message=FALSE}
...
```{r bench, cache=TRUE, message=FALSE}
...
```{r bench-scan, cache=TRUE, message=FALSE}
...
```{r fig_fpr, fig.cap="Fig 3. False regions called per rep (FPR-A) while true signal is present. All three methods sit around 0, meaning every call they make lands on a true region. The false-call rate under the null is the type I error in Table 1 below."}
par(mfrow=c(2,2), mar=c(4,4,2,1))
for (tr in traits) for (M in M_vals) {
