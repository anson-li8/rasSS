---
title: "Paper Replication (1000-SNP)"
output: rmarkdown::html_vignette
vignette: >
 %\VignetteIndexEntry{Paper Replication (1000-SNP)}
 %\VignetteEngine{knitr::rmarkdown}
 %\VignetteEncoding{UTF-8}
---

*The main simulation study, replicating the design of Jiang & Zhang (2025) on a simulated 1,000-SNP chromosome. Pilot and validation on the 300-SNP toy: [Pilot Study](pilot-study.html), [Validation](validation.html).*
...
Table: Total runtime seconds per rep, single core (includes CPD step, so see following for more clear difference)
...
Table: Algorithmic speedup, profile-generation time only (excludes the shared CPD step)
...
![Fig 3. False regions called per rep (FPR-A) while true signal is present. All three methods sit around 0, meaning every call they make lands on a true region. The false-call rate under the null is the type I error in Table 1 below.](figure/fig_fpr-1.png)
...
Table: Type I error at calibrated thresholds (exact 95% CI), 500 null reps per trait
...
Table: FPR-A: average false regions per rep (0 = nothing spurious)
...
Table: Power = detected ALL M true regions; low for M=3 because the CPD usually catches only 1-2 of the 3 peaks
